...
# -*- coding: utf-8 -*-
"""
Darkelf Cocoa Browser — r4b (macOS / PyObjC)
Fixes:
- JS toggle reloads the CURRENT page (no history jump)
- Tracker badge overlay reliably visible and updates
- Tabs always visible (tabbar brought to front after every WebView change)
- Address bar only searches on Return (no live-typing redirects)

Feature set:
- Embedded green homepage (inline SVG; DDG Lite search)
- Black toolbar; SF Symbols 22pt; hover -> green
- Hostname-only tab strip; active tab green rounded; + / ×
- Tracker blocker + green badge counter overlay
- Tor toggle (stem) + system SOCKS proxy via `networksetup`
- Keyboard shortcuts: ⌘T, ⌘W, ⌘R, ⌘L
- Storage cleanup on quit (cookies, localStorage, cache wiped)
- Enhanced spoofing: Battery, Client Hints, Performance fuzzing
- HTTPS-only + Onion guard unless Tor is active
"""
...
from WebKit import (
    WKWebView, WKWebViewConfiguration, WKUserContentController, WKUserScript, WKUserScript, WKPreferences, WKWebsiteDataStore
)
from Foundation import NSURL, NSURLRequest, NSMakeRect, NSNotificationCenter, NSDate
from AppKit import NSImageSymbolConfiguration, NSBezierPath, NSFont, NSAttributedString, NSAlert, NSAlertStyleCritical
...
class Browser(NSObject):
    ...
    def _make_toolbar(self):
        ...
        # --- Address/Search field (long) ---
        self.addr = NSSearchField.alloc().initWithFrame_(((0, 0), (1080, 32)))  # Slightly taller
        try:
            cell = self.addr.cell()
        ...
        try:
            ...
            self.addr.layer().setBorderColor_(NSColor.colorWithCalibratedRed_green_blue_alpha_(52/255.0, 199/255.0, 89/255.0, 1.0))
    ...
    def _update_tab_buttons(self):
        ...
        # Recreate one tab button + one image close button per tab
    ...
    def _new_wk(self) -> WKWebView:
        from WebKit import WKUserScript, WKUserScriptInjectionTime
        import secrets
        cfg = WKWebViewConfiguration.alloc().init()
        prefs = WKPreferences.alloc().init()
        try:
            prefs.setJavaScriptEnabled_(bool(self.js_enabled))
            prefs.setJavaScriptCanOpenWindowsAutomatically_(bool(self.js_enabled))
        except Exception: pass
        ...
        LETTERBOX_JS = r"""
        (function(){
          Object.defineProperty(window, 'innerWidth', {get: () => 1000, configurable: true});
          Object.defineProperty(window, 'innerHeight', {get: () => 1000, configurable: true});
          Object.defineProperty(window, 'outerWidth', {get: () => 1000, configurable: true});
          Object.defineProperty(window, 'outerHeight', {get: () => 1000, configurable: true});
          if (screen) {
            Object.defineProperty(screen, 'width', {get: () => 1000, configurable: true});
            Object.defineProperty(screen, 'height', {get: () => 1000, configurable: true});
            Object.defineProperty(screen, 'availWidth', {get: () => 1000, configurable: true});
            Object.defineProperty(screen, 'availHeight', {get: () => 1000, configurable: true});
          }
        })();
        """
    ...
    def _rebuild_active_webview(self):
        ...
        # 1. Remove all script message handlers from the old webview's userContentController
        try:
            ucc = old.configuration().userContentController()
            ucc.removeAllUserScripts()
            # Explicitly remove handlers if present
            for name in ["tracker", "panic"]:
                try: ucc.removeScriptMessageHandlerForName_(name)
                except Exception: pass
        except Exception:
            pass

        # 2. Remove old view from superview, and set to None for GC
        try:
        ...
        # 3. Remove badgeview from any superview before moving on
        try:
        ...
        # 4. Determine the URL to reload after rebuild
        url = ""
        try:
            u = old.URL()
            if u is not None:
        ...
        # 5. Create new WKWebView, attach, and store reference
        wk = self._new_wk()
        self.tabs[self.active].view = wk
        self._mount_webview(wk)

        # 6. Load the page or homepage
        if url in (None, "", "about:home", "about:blank", "about:blank#blocked", "about://home"):
    ...
    def _add_tab(self, url: str = "", home: bool = False):
        ...
        if home:
            ...
            self._load_url_in_active(url or "https://lite.duckduckgo.com/lite/")
    ...
    def _close_tab(self):
        ...
        try: cur.view.removeFromSuperview()
        except Exception: pass
        if not self.tabs:
            self._add_tab(home=True)
            return
        # choose new active index (left neighbor if needed)
    ...
    def actCloseTabIndex_(self, sender):
        ...
        if idx == self.active:
            try: self.tabs[idx].view.removeFromSuperview()
            except Exception: pass
    ...
    def actTor_(self, _):
        if TOR.proc is None:
            ok = TOR.start(); self.tor_on = bool(ok)
            if hasattr(self.btn_tor,"setContentTintColor_"):
                self.btn_tor.setContentTintColor_(NSColor.colorWithCalibratedRed_green_blue_alpha_(0.6,1.0,0.7,1.0) if ok else NSColor.whiteColor())
            if ok: print("[Tor] Running; system SOCKS proxy ON.")
        else:
            TOR.stop(); self.tor_on = False
            if hasattr(self.btn_tor,"setContentTintColor_"): self.btn_tor.setContentTintColor_(NSColor.whiteColor())
        try: self.tabs[self.active].view.reload_(None)
        except Exception: pass
    ...
    def actToggleJS_(self, _):
        ...
        try:
            sym = "bolt.slash" if not self.js_enabled else "bolt"
            img = NSImage.imageWithSystemSymbolName_accessibilityDescription_(sym, None)
            if img:
                img.setTemplate_(True)
                self.btn_js.setImage_(img)
                self.btn_js.setImagePosition_(2)  # ensure icon only
            self.btn_js.setToolTip_(f"JavaScript: {'ON' if self.js_enabled else 'OFF'}")
            # Set color
            if hasattr(self.btn_js, "setContentTintColor_"):
                if self.js_enabled:
    ...
    def actGo_(self, sender):
        try:
            ...
            # HTTPS-only + Onion guard
            url = ""
            is_onion = False
            if "://" not in text and "." not in text:
                url = "https://lite.duckduckgo.com/lite/?q=" + re.sub(r"\s+","+",text)
            elif "://" not in text:
                url = "https://" + text
            else:
                url = text
            ...
            if parsed:
                proto, host = parsed.group(1), parsed.group(2)
                is_onion = host.lower().endswith('.onion')
                if proto.lower() == "http":
                    if not self.tor_on:
                        self._show_block_alert("Plaintext HTTP is blocked.\nEnable Tor to access .onion or HTTP sites.")
                        return
                if is_onion:
                    if not self.tor_on:
                        self._show_block_alert(".onion sites are blocked unless Tor is running.")
                        return
            # <---- CHANGE HERE ---->
    ...
    def _storage_cleanup(self):
        try:
            # Wipe all website data (cookies, cache, localStorage)
            import WebKit
